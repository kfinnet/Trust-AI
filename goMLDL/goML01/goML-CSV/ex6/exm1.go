...
func main() {
	...
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepal_width", "petal_width"})
	fmt.Println(versicolorDF)

	// 데이터프레임을 필터링하고 다시 선택한다, 하지만 이번에는
	// 처음 세계의 결과만 보여준다
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepal_width", "petal_width"}).Subset([]int{6, 7, 8})
	fmt.Println(versicolorDF)
