...
func main() {
	...
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepal_length", "petal_length"})
	fmt.Println(versicolorDF)

	// 데이터프레임을 필터링하고 다시 선택한다, 하지만 이번에는
	// 처음 세계의 결과만 보여준다
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepal_length", "petal_length"}).Subset([]int{1, 2, 3})
	fmt.Println(versicolorDF)
	//fmt.Println(versicolorDF.strconv.Atoi(6,7,8))
}
